...
class runningScore(object):
    ...
    def get_scores(self):
        ...
        dice = (np.diag(hist)) / ((hist.sum(axis=1) + hist.sum(axis=0) - np.diag(hist))+np.diag(hist))  ##  2TP/(TP+FP+FN)+TP
...
def get_score_for_one_patient_cherhoo(labels, predicts, threshold=0.5):
    ...
    #print(labels.shape)
    chips = 189 - 3
    if labels.shape[0] != chips or predicts.shape[0] != chips:
        print('ERROR: 每个病人的切片数量应当是189！')
        return 0
    label_positive = labels > threshold
...
def get_score_from_all_slices_cherhoo(labels, predicts, threshold=0.5):
    ...
    '''
    chips = 189-3
    if labels.shape[0] % chips != 0 or predicts.shape[0] % chips != 0:
        print('ERROR: 输入切片数量应当是189的整数倍！')
        return np.array([])
    dice_scores = []
    iou_scores = []
    ...
    rvd_scores = []
    metrics_scores = []
    for i in range(labels.shape[0] // chips):
        tmp_labels = labels[i*chips : (i+1)*chips]
        tmp_pred = predicts[i*chips : (i+1)*chips]
...
def get_score_for_one_patient_cherhoo_depth(labels, predicts, threshold=0.5, depth=4):
    chips = 189 - depth + 1
    if labels.shape[0] != chips or predicts.shape[0] != chips:
        print('ERROR: 每个病人的切片数量应当是189！')
        return 0
...
def get_score_from_all_slices_cherhoo_depth(labels, predicts, threshold=0.5, depth=4):
    chips = 189-depth+1
    if labels.shape[0] % chips != 0 or predicts.shape[0] % chips != 0:
        print('ERROR: 输入切片数量应当是189的整数倍！')
        return np.array([])
    ...
    for i in range(labels.shape[0] // chips):
        tmp_labels = labels[i*chips : (i+1)*chips]
        tmp_pred = predicts[i*chips : (i+1)*chips]
        tmp_dice, tmp_iou, tmp_precision, tmp_recall, tmp_f1, tmp_voe, tmp_rvd, tmp_metrics = get_score_for_one_patient_cherhoo_depth(labels=tmp_labels, predicts=tmp_pred, threshold=threshold, depth=depth)
        dice_scores.append(tmp_dice)
        iou_scores.append(tmp_iou)
        precision_scores.append(tmp_precision)
        recall_scores.append(tmp_recall)
