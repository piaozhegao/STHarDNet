...
from compare_models_speed.ATLAS_models.lib.loader import CustomDataset_resize
from c_ATLAS.c_2d_3d_swin_hardnet.logger import get_logger
from compare_models_speed.ATLAS_models.lib.loss import DiceCE
from compare_models_speed.ATLAS_models.lib.trans_hardnet import TransHarDNet
...
def run(model_path, img_count):
    ...
    # train_data = DataLoader(CustomDataset_resize(train_data_list[:img_count], train_mask_list[:img_count]), batch_size=batch_size, shuffle=True, num_workers=4)
    ...
    ## create model
    model = TransHarDNet(n_channels=1, n_classes=2).cuda()
    ...
    train_data = DataLoader(CustomDataset_resize(train_data_list[:100], train_mask_list[:100]), batch_size=batch_size, shuffle=True, num_workers=4)
    start = time.time()
    ...
    train_data = DataLoader(CustomDataset_resize(train_data_list[:1000], train_mask_list[:1000]), batch_size=batch_size, shuffle=True, num_workers=4)
    start = time.time()
    start_2 = datetime.datetime.now()
    for i, pack in enumerate(train_data):
        images, labels = pack
    ...
    train_data = DataLoader(CustomDataset_resize(train_data_list[:10000], train_mask_list[:10000]), batch_size=batch_size, shuffle=True, num_workers=4)
    start = time.time()
    start_2 = datetime.datetime.now()
    for i, pack in enumerate(train_data):
        images, labels = pack
    ...
    train_data = DataLoader(CustomDataset_resize(train_data_list[:30000], train_mask_list[:30000]), batch_size=batch_size, shuffle=True, num_workers=4)
    start = time.time()
    start_2 = datetime.datetime.now()
    for i, pack in enumerate(train_data):
        images, labels = pack
    ...
    ##########################
    train_data = DataLoader(CustomDataset_resize(train_data_list[:50000], train_mask_list[:50000]), batch_size=batch_size, shuffle=True, num_workers=4)
    start = time.time()
    start_2 = datetime.datetime.now()
    for i, pack in enumerate(train_data):
        images, labels = pack
    ...
    train_data = DataLoader(CustomDataset_resize(train_data_list[:100000], train_mask_list[:100000]), batch_size=batch_size, shuffle=True, num_workers=4)
    start = time.time()
    start_2 = datetime.datetime.now()
    for i, pack in enumerate(train_data):
        images, labels = pack
...
if __name__ == '__main__':
    ...
    start = time.time()
    start_2 = datetime.datetime.now()
    model_path = r"C:\Users\user\Documents\python-code\11_c_paper\compare_models_speed\ATLAS_models\models\TransHarDNet_in2d_out2d"
    model_path = os.path.join(model_path, "c_TransHarDNet_256-224_by_id_best_model.pkl")
    # a = 1
    # for i in range(1000):
...
#
# C:\Users\user\.conda\envs\open-mmlab\python.exe C:/Users/user/Documents/python-code/11_c_paper/compare_models_speed/ATLAS_models/TransHarDNet_in2d_out2d.py
# 2021-10-22 00:26:05,584-<run>-[line:91]-INFO: Let the games begin
# 2021-10-22 00:26:05,584-<run>-[line:92]-INFO: ========================================
# 2021-10-22 00:26:05,584-<run>-[line:93]-INFO: # parameters:
# 2021-10-22 00:26:05,584-<run>-[line:94]-INFO: IMG_H=224
# 2021-10-22 00:26:05,584-<run>-[line:95]-INFO: IMG_W=224
# 2021-10-22 00:26:05,584-<run>-[line:96]-INFO: img_depth=1
# 2021-10-22 00:26:05,584-<run>-[line:97]-INFO: img_depth_out_num=2
# 2021-10-22 00:26:05,584-<run>-[line:98]-INFO: batch_size=16
# 2021-10-22 00:26:05,584-<run>-[line:99]-INFO: n_classes=2
# 2021-10-22 00:26:05,584-<run>-[line:100]-INFO: learning_rate=0.001
# 2021-10-22 00:26:05,584-<run>-[line:101]-INFO: patience_scheduler=5
# 2021-10-22 00:26:05,584-<run>-[line:102]-INFO: patience_early_stop=10
# 2021-10-22 00:26:05,584-<run>-[line:103]-INFO: ReduceLR_factor=0.2
# 2021-10-22 00:26:05,584-<run>-[line:105]-INFO: ========================================
# ==========================================
# Device: cuda
# Current cuda device: 0
# torch.cuda.device_count()= 1
# ==========================================
# 2021-10-22 00:26:05,726-<run>-[line:132]-INFO: ----------------------------------------- load train (33453): C:/Users/user/Documents/python-code/11_c_paper/data/ATLAS/by_id/train/imgs/
# Parameters: 27022264
# Parameters: 27022264
# predict 100 imgs : 5.183484077453613
# predict 100 imgs : 0:00:05.183484
# predict FPS is      : 19.292043441392224/s
# ------------------------------------------------
# predict 1,000 imgs : 6.2767333984375
# predict 1,000 imgs : 0:00:06.276734
# predict FPS is      : 159.318539839359/s
# ------------------------------------------------
# predict 10,000 imgs : 33.23492908477783
# predict 10,000 imgs : 0:00:33.234929
# predict FPS is      : 300.88826049519605/s
# ------------------------------------------------
# predict 30,000 imgs : 93.97801756858826
# predict 30,000 imgs : 0:01:33.978018
# predict FPS is      : 319.22358841103465/s
# ------------------------------------------------
# all:140.3546905517578
# all:0:02:20.354690
#
