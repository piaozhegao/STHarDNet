...
from compare_models_speed.ATLAS_models.lib.DUnet import DUnet
...
def run(model_path, img_count):
    ...
    train_mask_list.extend(train_mask_list)
    train_data = DataLoader(CustomDataset_3d_gray_out_1(train_data_list, train_mask_list, img_h=192, img_w=192, out_num=img_depth_out_num), batch_size=batch_size, shuffle=True, num_workers=4 * 2)
    logger.info("----------------------------------------- load train ({}): {}".format(len(train_data_list), train_img_path))


    # train_data = DataLoader(CustomDataset_3d_gray_out_1(train_data_list[:img_count], train_mask_list[:img_count]), batch_size=batch_size, shuffle=True, num_workers=4)
    ...
    ## create model
    model = DUnet(in_channels=img_depth).cuda()
    ...
    summary_(model, (4, 192, 192), batch_size=batch_size)
    total_params = sum(p.numel() for p in model.parameters())
    logger.info('Parameters:{}'.format(total_params))

    optimizer = Adam(model.parameters(), lr=learning_rate)

    train_data = DataLoader(CustomDataset_3d_gray_out_1(train_data_list[:100], train_mask_list[:100], img_h=192, img_w=192, out_num=img_depth_out_num), batch_size=batch_size, shuffle=True, num_workers=4)
    start = time.time()
    ...
    train_data = DataLoader(CustomDataset_3d_gray_out_1(train_data_list[:1000], train_mask_list[:1000], img_h=192, img_w=192, out_num=img_depth_out_num), batch_size=batch_size, shuffle=True, num_workers=4)
    start = time.time()
    start_2 = datetime.datetime.now()
    for i, pack in enumerate(train_data):
        images, labels = pack
    ...
    train_data = DataLoader(CustomDataset_3d_gray_out_1(train_data_list[:10000], train_mask_list[:10000], img_h=192, img_w=192, out_num=img_depth_out_num), batch_size=batch_size, shuffle=True, num_workers=4)
    start = time.time()
    start_2 = datetime.datetime.now()
    for i, pack in enumerate(train_data):
        images, labels = pack
    ...
    train_data = DataLoader(CustomDataset_3d_gray_out_1(train_data_list[:30000], train_mask_list[:30000], img_h=192, img_w=192, out_num=img_depth_out_num), batch_size=batch_size, shuffle=True, num_workers=4)
    start = time.time()
    start_2 = datetime.datetime.now()
    for i, pack in enumerate(train_data):
        images, labels = pack
    ...
    ####################################
    train_data = DataLoader(CustomDataset_3d_gray_out_1(train_data_list[:50000], train_mask_list[:50000], img_h=192, img_w=192, out_num=img_depth_out_num), batch_size=batch_size, shuffle=True, num_workers=4)
    start = time.time()
    start_2 = datetime.datetime.now()
    for i, pack in enumerate(train_data):
        images, labels = pack
    ...
    train_data = DataLoader(CustomDataset_3d_gray_out_1(train_data_list[:100000], train_mask_list[:100000], img_h=192, img_w=192, out_num=img_depth_out_num), batch_size=batch_size, shuffle=True, num_workers=4)
    start = time.time()
    start_2 = datetime.datetime.now()
    for i, pack in enumerate(train_data):
        images, labels = pack
...
if __name__ == '__main__':
    ...
    start = time.time()
    start_2 = datetime.datetime.now()
    model_path = "C:/Users/user/Documents/python-code/11_c_paper/compare_models_speed/ATLAS_models/models/D-UNet_in2d_out2d"
    model_path = os.path.join(model_path, "c_DUnet_192_ATLAS_best_model.pkl")
    # a = 1
    # for i in range(1000):
